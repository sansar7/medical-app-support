...
parser.add_argument('--source-root', type=Path, help='Optional parent directory containing the four app repositories')
...
def inline(s):
 s=escape(s)
 s=re.sub(r'\[([^\]]+)\]\(([^)]+)\)',lambda m:f'<a href="{m[2] if m[2] != "PRIVACY_POLICY.md" else "privacy.html"}">{m[1]}</a>',s)
...
for slug,name,repo in apps:
 ...
 for source,filename in [('PRIVACY_POLICY.md','privacy.html'),('SUPPORT.md','index.html')]:
  p=(base/repo/source) if base else (dest/source);text=p.read_text();manifest[slug+'/'+source]=hashlib.sha256(p.read_bytes()).hexdigest()
